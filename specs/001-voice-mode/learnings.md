...
- **T009**: `uv run pytest` fails because pytest isn't on PATH — use `uv run python -m pytest` or `uv pip install pytest` first. The `[project.optional-dependencies] dev` group isn't auto-installed by `uv sync --dev`; need explicit `uv pip install pytest pytest-asyncio`.
