...
- [ ] T001 Create `flake.nix` with devShell providing nodejs_22, python311, uv, portaudio headers. Create `.envrc` with `use flake`. Create `.gitignore` covering: `.direnv/`, `node_modules/`, `dist/`, `out/`, `.vscode-test/`, `*.vsix`, `__pycache__/`, `*.pyc`, `.pytest_cache/`, `.mypy_cache/`, `.ruff_cache/`, `*.egg-info/`, `.uv/`, `*.onnx` (downloaded at runtime), `flake.lock`, `.env`, `coverage/`. [FR-092]
