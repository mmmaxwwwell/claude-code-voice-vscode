...
- [x] T043 [P] Create custom test reporters for structured output: Vitest custom reporter producing `test-logs/unit-ts/<timestamp>/summary.json` + `failures/<test-name>.log`. pytest plugin producing `test-logs/unit-python/<timestamp>/summary.json` + `failures/<test-name>.log`. Each failure log includes assertion details, stack trace, and context. [FR-150, FR-151, FR-152]
...
- [x] T037 [P] Create a "hey claude" openWakeWord model using the ONNX format (not TFLite — tflite_runtime is incompatible with numpy 2.x in this env). Steps: (1) Add `onnx` as a dev dependency in pyproject.toml if not already present. (2) Write `scripts/generate_hey_claude_model.py` that uses the `onnx` Python package (`onnx.helper`) to programmatically build a valid ONNX model with input shape `[1, 16, 96]` (float32) and output shape `[1, 1]` (float32) — matching the architecture of openWakeWord's built-in models (e.g. `alexa_v0.1.onnx`). The scaffold model should have a simple dense layer (Flatten → MatMul → Add → Sigmoid) with random weights — it won't detect "hey claude" but is structurally valid for pipeline integration. (3) Run the script to generate `models/hey_claude.onnx`. (4) Validate: load the model with `onnxruntime.InferenceSession` and confirm input/output shapes match. Also validate it loads via `openwakeword.Model(wakeword_models=["models/hey_claude.onnx"], inference_framework="onnx")`. (5) Update `sidecar/wakeword.py` to pass `inference_framework="onnx"` when constructing `openwakeword.Model`, and change the default model file extension from `.tflite` to `.onnx`. (6) Update the `claude-voice.wakeWordModelPath` config default and any references to `hey_claude.tflite` in the codebase to `hey_claude.onnx`. (7) Commit the model to `models/hey_claude.onnx`. Environment note: Python C extensions (numpy, onnxruntime) need LD_LIBRARY_PATH. Prefix ALL Python/uv commands with: `LD_LIBRARY_PATH=/nix/store/ab3753m6i7isgvzphlar0a8xb84gl96i-gcc-15.2.0-lib/lib:/nix/store/2kdz3m7ic8w226pcvkz1dlg169v91p6a-zlib-1.3.2/lib uv run python ...` — Do NOT use `nix develop`, `find /nix/store`, `nix eval`, or `ldconfig`. The paths above are correct and stable.
...
- [ ] T040 Update `CLAUDE.md` with final development guide, architecture overview, test guide, troubleshooting. (Re-run after T037 to reflect ONNX model changes, updated flake.nix LD_LIBRARY_PATH, and any wakeword.py changes.)
...
- **Phase 10**: T037 and T038 can run in parallel. T040 depends on T037 (must reflect ONNX changes). REVIEW depends on T037+T040.
...
- The `models/hey_claude.onnx` file (T037) is a blocker for real wake word integration tests. Until it exists, wake word tests can use one of openWakeWord's built-in models (e.g., `hey_jarvis`) as a stand-in. Note: use ONNX format, not TFLite — `tflite_runtime` is incompatible with numpy 2.x in this NixOS environment. openWakeWord supports both formats and auto-switches to onnxruntime when `.onnx` files are provided.
